...
export function BottomNav() {
  ...
  const navItems = [
    {
      href: "/",
      icon: (
        <svg className="w-7 h-7" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
      <div className="relative flex items-center p-2 bg-white dark:bg-zinc-900 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgb(255,255,255,0.05)] border border-zinc-100 dark:border-zinc-800 backdrop-blur-md bg-white/80 dark:bg-zinc-900/80">

        {/* Moving Pill Background */}
        <div
          className="absolute left-2 top-2 bottom-2 w-16 rounded-full transition-transform duration-300 ease-[cubic-bezier(0.4,0,0.2,1)]"
          style={{
        ...
        {navItems.map((item, idx) => {
          ...
          return (
            <Link
              ...
              href={item.href}
              className={cn(
                "relative flex items-center justify-center w-16 h-14 rounded-full z-10 transition-colors duration-300",
